...
pair<int,int> findCoordinates(long long idx){
    pair<int,int> answer = { 0,0 };
    if(idx == 0) {
        return answer;
    }
    
    answer.first  = 1 + (idx - 1) % boardSize;
    answer.second = 1 + (idx - 1) / boardSize;
    
    // if the second coordinate is 0, this mean it is on the next row
...
void bfs(){
    for(int i = 1; i <= ind.size(); i++){
        // only need to start filling from the ending points and move in reverse
        if(!startingPoint[i]) {
            queue<int> q;
            q.push(i);
            while(!q.empty()){
...
void read(int x){
    for(int i = 0; i < x; i++) {
        ...
        for(int j = 0; j < 2; j++) {
            ...
            if(ind[position[j]] == 0) {
                ind[position[j]] = ind.size() + 1;
                inv[ind.size()] = position[j];
            }
        }
